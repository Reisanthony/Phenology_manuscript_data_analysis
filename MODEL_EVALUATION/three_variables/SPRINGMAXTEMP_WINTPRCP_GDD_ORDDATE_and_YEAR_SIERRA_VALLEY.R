...
bb<-as.matrix(cbind(stand(sub_dat$spring_tmax), stand(sub_dat$winter_prcp), stand(sub_dat$estGdd), stand(sub_dat$estGdd)^2, 
...
D<-list(X=bbi,N=dim(bbi)[1],K=13,L=length(sp),y=sub_dat$pa,ll=as.numeric(as.factor(sub_dat$genus_species)))
fit<-stan("hmodel_without_forecast.stan",data=D,iter=4000,warmup=2000) ## default is warmup = 1/2 iter

save(list=ls(),file=paste("SPRINGMAXTEMP_WINTPRCP_GDD_ORDDATE_and_YEAR_",sites[j],".rdat",sep=""))
