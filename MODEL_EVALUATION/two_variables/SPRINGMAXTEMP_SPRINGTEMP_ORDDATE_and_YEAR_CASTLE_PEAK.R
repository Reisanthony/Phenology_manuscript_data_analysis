...
bb<-as.matrix(cbind(stand(sub_dat$spring_tmax), stand(sub_dat$spring_tmin), stand(sub_dat$ordDate), stand(sub_dat$ordDate)^2, 
                    stand(sub_dat$Year)))
...
save(list=ls(),file=paste("SPRINGMAXTEMP_SPRINGTEMP_ORDDATE_and_YEAR_",sites[j],".rdat",sep=""))
