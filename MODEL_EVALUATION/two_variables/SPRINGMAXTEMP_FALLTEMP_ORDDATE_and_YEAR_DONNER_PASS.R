...
bb<-as.matrix(cbind(stand(sub_dat$spring_tmax), stand(sub_dat$prevfall_tmin), stand(sub_dat$ordDate), stand(sub_dat$ordDate)^2, 
                    stand(sub_dat$Year)))
...
save(list=ls(),file=paste("SPRINGMAXTEMP_FALLTEMP_ORDDATE_and_YEAR_",sites[j],".rdat",sep=""))
