...
# Remove species with U curve
sp_rm <- c(sp[1], sp[19], sp[31], sp[32], sp[42]) 
sub_dat<-sub_dat[ (!sub_dat$genus_species %in% sp_rm),]
sp<-unique(sub_dat$genus_species)

# Covariate matrix
bb<-as.matrix(cbind(stand(sub_dat$ordDate), stand(sub_dat$ordDate)^2, stand(sub_dat$Year)))
bbi<-as.matrix(cbind(bb, bb[,1]*bb[,3], bb[,2]*bb[,3]))

# Stan data and model fitting
D<-list(X=bbi,N=dim(bbi)[1],K=5,L=length(sp),y=sub_dat$pa,ll=as.numeric(as.factor(sub_dat$genus_species)))
fit<-stan("hmodel_without_forecast.stan",data=D,iter=4000,warmup=2000) ## default is warmup = 1/2 iter

save(list=ls(),file=paste("YEAR_PHENOLOGY_INTERACTION_",sites[j],".rdat",sep=""))
