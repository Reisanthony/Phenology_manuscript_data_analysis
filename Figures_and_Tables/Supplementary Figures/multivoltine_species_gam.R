...
pdf(paste("multivoltine_species_gam_Castle_Peak.pdf",sep=""),width=9,height=9)
par(mfrow = c(3, 3))
...
for (i in 1:length(spmulti)) {
  ...
  for (j in 1:length(year)) {
    ...
    if (length(b) > 6) {  # only fit GAM if enough data
      ...
      fit <- gam(pa ~ s(ordDate, k = 5), data = datj, family = binomial)  
...
pdf(paste("multivoltine_species_gam_Donner_Pass.pdf",sep=""),width=9,height=9)
par(mfrow = c(3, 3))
par(mar = c(4.5, 5.5, 2.5, 1.5))
...
pdf(paste("multivoltine_species_gam_Lang_Crossing.pdf",sep=""),width=9,height=9)
...
pdf(paste("multivoltine_species_gam_Sierra_Valley.pdf",sep=""),width=9,height=9)
...
pdf(paste("multivoltine_species_gam_Washington.pdf",sep=""),width=9,height=9)
